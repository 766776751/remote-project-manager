...
async function openDatabase() {
  // 优先级：环境变量 DB_PATH（飞牛等容器部署可指向持久卷）> Vercel /tmp > 用户目录。
  let live;
  if (process.env.DB_PATH) {
    live = path.resolve(BASE_DIR, process.env.DB_PATH);
  } else if (IS_VERCEL) {
    live = path.join('/tmp', 'remote-project-manager-data.db');
  } else {
    live = path.join(os.homedir(), 'remote-project-manager-data', 'data.db');
  }
  LIVE_DB_PATH = live;
